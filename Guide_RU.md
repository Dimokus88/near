...
После чего, нажмите `UPDATE DEPLOYMENT` (или `CREATE DEPOYMENT`), подтвердите транзакцию и ваш кантейнер будет развернут уже как нода валидатора. И если контейнер будет перезагружен - то ваш `validator_key.json` будет скачан и нода корректно запущена. 
